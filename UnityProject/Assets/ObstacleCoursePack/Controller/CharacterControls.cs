...
public class CharacterControls : MonoBehaviour {
		...
		if (canMove)
		{
			...
			if (IsGrounded())
			{
			 // Calculate how fast we should be moving
				...
				// Jump — flag set in Update() so wasPressedThisFrame is always caught
	...
	private void Update()
	{
		...
		float h = kb == null ? 0f : (kb.dKey.ReadValue() - kb.aKey.ReadValue());
		float v = kb == null ? 0f : (kb.wKey.ReadValue() - kb.sKey.ReadValue());

		Vector3 v2 = v * cam.transform.forward;
		Vector3 h2 = h * cam.transform.right;
		moveDir = (v2 + h2).normalized;

		// Capture jump in Update so wasPressedThisFrame is never missed by FixedUpdate
